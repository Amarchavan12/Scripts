#!/bin/bash
#Set your project ID
PROJECT_ID="totemic-carrier-297406"
gcloud config set project $PROJECT ID

#List of VPCs to check
VPCS=("ibmi-gpay-dmz-out-trust-vpc01" "ibmi-gpay-dmz-out-untrust-vpc01" "ibmi-gpay-dmz-untrust-vpc01")

# IP to be removed
read -p "Enter The IP address to be removed from firewall rule: "IP_TO_REMOVE #IP TO REMOVE="104.223.91.19"
...
remove_ip_from_firewall() {
	...
	echo "Backing up source ranges for firewall rule '$firewall_name" echo "$source_ranges_comma" > "${firewall_name}_backup.txt"
...
#List all firewall rules in the project
	gcloud compute firewall-rules list --project="$PROJECT_ID" --format-"csv[no-heading] (name, network, sourceRanges)" | while IFS-, read -r firewall_nam network source_ranges; do
